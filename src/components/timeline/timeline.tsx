...
const TimelineItem = React.forwardRef<HTMLLIElement, TimelineItemProps>(
  (
    ...
    const commonClassName = cn(
      'relative w-full mb-6 last:mb-0',
      className,
    );
      ...
      >
        ...
        <div className="flex flex-col items-center h-[-webkit-fill-available]">
          <div className="relative z-10">
            <TimelineIcon icon={icon} color={iconColor || 'primary'} status={status} iconSize={iconsize || 'md'} />
          </div>
          {showConnector && (
...
>(({ className, ...props }, ref) => (
  <p ref={ref} className={cn('max-w-sm text-sm text-muted-foreground w-full max-w-full', className)} {...props} />
));
TimelineDescription.displayName = 'TimelineDescription';

const TimelineContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn('flex flex-col gap-2 pl-2 flex-1', className)} {...props} />
  ),
);
